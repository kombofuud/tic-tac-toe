...
std::vector<std::vector<std::vector<float>>> makeResBlock(int input, int output){
    ...
    block.push_back(makeMat(input + 1, input)); //first nn layer of the block the +1 is to account for the bias term
    block.push_back(makeMat(input + 1, output)); //second nn layer
